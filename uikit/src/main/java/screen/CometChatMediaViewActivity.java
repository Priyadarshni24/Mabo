...
import static utils.Utils.isFilePresent;

public class CometChatMediaViewActivity extends AppCompatActivity {

    private ImageView imageMessage;
    private FullscreenVideoView videoMessage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mediaType.equals(CometChatConstants.MESSAGE_TYPE_IMAGE)) {
            ...
            if (isFilePresent(videoFilepath)){
                videoMessage.videoFile(new File(videoFilepath));

            }else{
                videoMessage.videoUrl(mediaUrl);
            }
            videoMessage.enableAutoStart();
           /* if (isFilePresent(videoFilepath))
            {
                MediaController mediacontroller = new MediaController(this);
                mediacontroller.setAnchorView(videoMessage);
                videoMessage.setMediaController(mediacontroller);
                videoMessage.setVideoURI(Uri.parse(videoFilepath));
